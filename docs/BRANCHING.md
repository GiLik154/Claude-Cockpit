...
이 문서는 Claude Cockpit의 브랜치/버전/릴리즈 정책을 정의합니다.

## 브랜치 모델

| 브랜치 | 역할 | 라이프사이클 |
|--------|------|-------------|
| `main` | 트렁크. 모든 신규 기능/리팩토링/일반 버그 수정이 머지되는 라인 | 영속 |
| `release/vX.Y.Z` | 정식 릴리즈 시점의 스냅샷. 해당 버전 hotfix 라인 | 영속 (해당 버전 사용자가 있는 한) |
| `feature/*`, `fix/*` | 작업용 브랜치 (선택) | PR 머지 후 삭제 |

릴리즈 태그 `vX.Y.Z`는 항상 같은 이름의 `release/vX.Y.Z` 브랜치 HEAD에 부여합니다.
...
이후 `main`에서 다음 사이클 개발을 계속 진행합니다.
...
# 3) PATCH 버전을 위한 새 release 브랜치 분기
#    (release/v1.1.0은 v1.1.0 시점 스냅샷으로 보존)
git checkout -b release/v1.1.1
git push -u origin release/v1.1.1

# 4) 태그 부여
git tag -a v1.1.1 -m "v1.1.1: <요약>"
git push origin v1.1.1

# 5) main에도 동일 변경이 반영되어 있는지 확인
#    - 이미 포함하면 끝
#    - 아니면 main에도 적용
```

> **PATCH마다 새 `release/vX.Y.Z` 브랜치**를 만들어 각 버전의 스냅샷을
> 명확히 보존합니다. 같은 minor 라인에서 후속 hotfix가 또 나오면
> 직전 PATCH 브랜치(`release/v1.1.1`)에서 분기해 다음 브랜치
> (`release/v1.1.2`)를 만듭니다.
...
- ruff lint / format (informational)

대상 브랜치: `main`, `release/**`. PR이 머지되기 전에 통과해야 합니다.
